...
p1a <- make_conv_panel(agg_alpha_B, expression(alpha[B] ~ "(message weight)"),
                       NASH_ALPHA_B, "Nash: 0", c(0, 1), show_legend = TRUE)
p1b <- make_conv_panel(agg_tau_B,   expression(tau[B] ~ "(client threshold)"),
                       NASH_TAU_B,  "Nash: 1", c(1, 10), show_legend = FALSE)
p1c <- make_conv_panel(agg_delta_B, expression(delta[B] ~ "(client discount)"),
                       NASH_DELTA_B, "Nash: 0", c(0, 1), show_legend = FALSE)
...
p3a <- make_payoff_panel(agg_cli_pay, "Client payoff",
                         RAND_CLI, sprintf("Random play: %.1f", RAND_CLI))
p3b <- make_payoff_panel(agg_adv_pay, "Biased advisor payoff",
                         RAND_ADV, sprintf("Random play: %.1f", RAND_ADV))
...
# --- additional aggregation: tau_no and delta_no for BOTH treatments ---
agg_tau_no   <- two_stage_avg(df_B, "tau_no")
agg_delta_no <- two_stage_avg(df_B, "delta_no")

# --- figure 4: client behaviour under no disclosure (and type-B periods, both treatments) ---
make_conv_panel_nonash <- function(agg_data, y_label, y_limits = NULL, show_legend = TRUE) {
  p <- ggplot(agg_data, aes(x = period, y = mean, colour = treatment, fill = treatment)) +
    geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.15, colour = NA) +
    geom_line(linewidth = 0.8) +
    col_scale_c + col_scale_f +
...
# --- figure 4: client behaviour under no disclosure (and type-B periods, both treatments) ---
p4a <- make_conv_panel_nonash(agg_tau_no, expression(tau[no] ~ "(client threshold)"),
                              c(1, 10), show_legend = TRUE)
p4b <- make_conv_panel_nonash(agg_delta_no, expression(delta[no] ~ "(client discount)"),
                              c(0, 1), show_legend = FALSE)
...
ggsave(file.path(output_dir, "fig4_client_no_disclosure.pdf"),  plot = fig4, width = 8,  height = 4.5)
ggsave(file.path(output_dir, "fig4_client_no_disclosure.png"),  plot = fig4, width = 8,  height = 4.5, dpi = 300)

cat("  fig4_client_no_disclosure.pdf / .png\n")