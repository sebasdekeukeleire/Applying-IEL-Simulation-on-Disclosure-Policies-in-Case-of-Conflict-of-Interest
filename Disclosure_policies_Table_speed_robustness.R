...
# --- Welch t-test: TOTAL WELFARE (all interactions, periods 41-50) ---
...
summarise_run <- function(filepath) {
  ...
  list(
    ...
    tau_no     = s1_mean(late_B, "tau_no"),
    delta_no   = s1_mean(late_B, "delta_no"),
    tau_B      = s1_mean(late_B, "tau_B"),
    delta_B    = s1_mean(late_B, "delta_B"),
...
rob_rows <- lapply(robustness_runs, function(run) {
  ...
  data.frame(
    ...
    tau_no_no  = round(no$tau_no,     3),
    tau_no_ma  = round(ma$tau_no,     3),
    dno_no     = round(no$delta_no,   3),
    dno_ma     = round(ma$delta_no,   3),
    tau_B_ma   = round(ma$tau_B,      3),
